...
void readLineArray()
{
  for(int lettura=0;lettura<8;lettura++)
  {
    byte sensoriNuovi = 0;
    if(digitalRead(linee[lettura]))
    {
      ...
      if(nSens<4)
      {
        for(int i=0;i<4;i++)
        {
          if(lettura!=sensoriTrovati[i]) sensoriNuovi++;

        }
        //Serial.println(sensoriNuovi);
        if(sensoriNuovi==4)
        {
          if(Portiere) portiereLineaPresa=1,portiereLineaMillis=millis();
...
void checkLines()
{
  ...
  if(RILEVA_BORDI)
  {
    ...
    if(nSens>0)
    {
      ...
      //display.fillRect(0,0,128,128,RED);
      //direzioneCorrezione = double((atan2(Y,X)*180/PI)+180);
      vel = VELOCITA_FUGA_LINEA;
    }
    currentMillis = millis();
    if(currentMillis - correzioneMillis>=TEMPO_FUGA && stoCorreggendo)
    {
      for(int i=0;i<4;i++)
      {
        sensoriTrovati[i] = 90;
      ...
      Y = 0;
      stoCorreggendo = 0;
    }//if(currentMillis - correzioneMillis>=TEMPO_FUGA)
  }//if(RILEVA_BORDI)
}// void checkLines()
