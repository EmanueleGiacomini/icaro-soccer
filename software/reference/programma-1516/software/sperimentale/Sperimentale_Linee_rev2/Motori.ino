...
void moveRobot(int angolo, int rotazione, int vel, boolean abilitaCorrezione)
{
  ...
  if(angolo == -1)// fermo
  { 
    if(!stoCorreggendo && RILEVA_BORDI)
    {
      individuaLinee();
      cmdMot(0,0);
      cmdMot(1,0);
  ...
  else
  {
    ...
    if (abilitaCorrezione)
    {
      ...
      if (usoPixy)
      {
        Serial.println("USO PIXY");
        kp = 0.2;
        //ki = 0;
        kd = 0.05;
        //bussola_getHeadingPIXY();
        //Input = gradiE;
        //setPoint = 0;
        //myPID.SetControllerDirection(DIRECT);
        //tone(25,6000);
        Serial.print("vnord magic smoke: ");
        Serial.print(vNordMagicSmoke);
        Serial.print("\t errore:  ");
        Serial.println(vNordMagicSmoke);
        corre= vNordMagicSmoke;
      }
      else
      {
        kp = 0.95;
    ...
    if(SOFT_MOVES)
    {
      volatile int SvelZero,SvelUno,SvelDue,SvelTre;


      if(currentMillis - softMovesMillis > SOFT_MOVES_INTERVALLO)
      {
        SvelZero = constrain(SvelZero, SvelZero-20, SvelZero+20);
        cmdMot(0,constrain(constrain(velZero, velZero-25,velZero+25),-vel,vel));
        Serial.print(velZero);
        Serial.print(" - ");
        cmdMot(1,constrain(constrain(velUno, velUno-25,velUno+25),-vel,vel));
        Serial.print(velUno);
        Serial.print(" - ");
        cmdMot(2,constrain(constrain(velDue, velDue-25,velDue+25),-vel,vel));
        Serial.print(velDue);
        Serial.print(" - ");
        cmdMot(3,constrain(constrain(velTre, velTre-25,velTre+25),-vel,vel));
        Serial.println(velTre);
        softMovesMillis = millis();
      }//if(currentMillis - softMovesMillis > intervalloSoft) 
    }//if(SOFT_MOVES)
    else
    {
    ...
    //Serial.print(stoCorreggendo),Serial.print(" - "),Serial.println(RILEVA_BORDI);
    if(!stoCorreggendo && RILEVA_BORDI)
    {
      individuaLinee();
    }// if(!stoCorreggendo)
...
void setSpeed(byte motore, int targetSpeed)
{
