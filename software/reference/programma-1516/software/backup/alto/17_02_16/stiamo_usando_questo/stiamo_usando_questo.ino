...
int intervallo_Linea = 1000,intervallo_correzione = 500; // Timer per lettura della linea
...
void loop()
{
  ...
  leggiSerialeUno();
  if(!posPalla || posPalla == 1 || posPalla == 19) traiettoriaPalla = 0, vel = 170;
  if(posPalla != 99 && (posPalla > 1 && posPalla <= 7))
  {
    traiettoriaPalla = (90/8)*(posPalla+4);
  }

  if(posPalla != 99 && (posPalla >=13 && posPalla < 19))
  {
    traiettoriaPalla = -(90/8)*((20-posPalla)+4);
  }

  if(posPalla != 99 && (posPalla > 7 && posPalla <= 12))
  {
    traiettoriaPalla = 30*(sensPalla+4);
  }
