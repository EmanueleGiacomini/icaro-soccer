...
SSSSSSSSSSSSSSS   llllllll  aaaaaaaaaa  aaaa        vvv            eeeeeeeeeeeeee
   ...
   _____ _____  ______ _____  _____ __  __ ______ _   _ _______       _      ______     ____           _____ _____  ____     _____  ________      __  ____  
  / ____|  __ \|  ____|  __ \|_   _|  \/  |  ____| \ | |__   __|/\   | |    |  ____|   |  _ \   /\    / ____/ ____|/ __ \   |  __ \|  ____\ \    / / |___ \ 
 | (___ | |__) | |__  | |__) | | | | \  / | |__  |  \| |  | |  /  \  | |    | |__      | |_) | /  \  | (___| (___ | |  | |  | |__) | |__   \ \  / /    __) |
  \___ \|  ___/|  __| |  _  /  | | | |\/| |  __| | . ` |  | | / /\ \ | |    |  __|     |  _ < / /\ \  \___ \\___ \| |  | |  |  _  /|  __|   \ \/ /    |__ < 
  ____) | |    | |____| | \ \ _| |_| |  | | |____| |\  |  | |/ ____ \| |____| |____    | |_) / ____ \ ____) |___) | |__| |  | | \ \| |____   \  /     ___) |
 |_____/|_|    |______|_|  \_\_____|_|  |_|______|_| \_|  |_/_/    \_\______|______|   |____/_/    \_\_____/_____/ \____/   |_|  \_\______|   \/     |____/ 
...
unsigned long prevMillisBenchmark = 0;
unsigned long currentMillis = 0;

#define PRINT_VALORI_PALLA

unsigned long prevMillisPalla = 0;
...
unsigned int contatoreMediaPalla;
unsigned int contatoreCalibrazione;



// SENSORE DISTANZA PORTIERE
int sharpSensor = 30;
bool ostacoloRilevato = 0;
...
void getPortaDistance()
{
  int sharpValue = analogRead(sharpSensor);
  if(sharpValue>600) ostacoloRilevato = 1;
  else ostacoloRilevato = 0;
}
...
void loop()
{
  ...
  if (currentMillis - prevMillisPalla > 3) {

    for (int i = 0; i <= 19; i++) {
      valPalla[i] = valPalla[i] /  contatoreMediaPalla;
    }
    contatoreMediaPalla = 0;
    posPalla();  //// aggiorno i valori palla e distanza
  ...
  if (Serial1.available()) {
    ...
    if (istruzione == byteRichiesta) {  // byte richiesta = 'r'
      ...
      Serial1.write(ostacoloRilevato); //SHARP 
      Serial1.write(0);
      Serial1.write(0);
      Serial1.write(0); //us
  ...
  if (currentMillis - prevMillisBenchmark > 1000) {
    Serial.print(loopsPerSec);
    Serial.println(" loops per second");
    loopsPerSec = 0;
    prevMillisBenchmark = currentMillis;
  }
#endif
