...
{ 
  ...
  // Se dopo un secondo dall'ultima lettura non ho rilevato nulla, azzero l'array
  if(currentMillis - sensorePresoMillis > 1000 && !hoAzzerato)
  {
    display.clearScreen();
    rgb(0,0,0);
    Serial.println("Sto Azzerando");
    for(int i=0;i<36;i++)
    {
      //noTone(25);
      sensoriTrovati[i] = 0;
  ...
  while(currentMillis - sensorePresoMillis < 1000)
  {
    stoCorreggendo = 1;
    currentMillis = millis();
    Serial.print(currentMillis),Serial.print(" - "),Serial.println(sensorePresoMillis);
    leggiArrayLinee();
    calcolaDirezione();
    moveRobot(direzioneCorrezione,0,100,1);
  }//while(currentMillis - sensorePresoMillis < 1000)
...
void leggiArrayLinee()
{
  for(int lettura=0;lettura<12;lettura++)
  {
    ...
    {
//      sensoriTrovati[lettura+(nMux*12)] = lineaRilevata(nMux,lettura);
      if(lineaRilevata(nMux,lettura))
      {
        if(!sensoriTrovati[lettura+(nMux*12)] && lettura+nMux*12 != 22)
        {
          rgb(255,0,0);
          display.clearScreen();
          ...
          display.print(lettura+nMux*12);
          //tone(25,5000);
          //Serial.println(lettura+(nMux*12));
          sensoriTrovati[lettura+(nMux*12)] = 1,hoAzzerato = 0,sensorePresoMillis=millis();
        }
        else sensorePresoMillis = millis();
      }
    }
  }
}
...
void calcolaDirezione()
{
  ...
  for(i=0;i<36;i++)
  {
    X += cos_Linee[i]*sensoriTrovati[i];
    Y += sin_Linee[i]*sensoriTrovati[i];
  }//  for(i=0;i<36;i++)
  if(!X && !Y)
  {
    return;
  }
  direzioneCorrezione = double((atan2(Y, X) * 180 / PI) + 180);
...
void individuaLinee()
{
  ...
  if (stoCorreggendo)
  {
    ...
    while ((currentMillis - correzioneMillis < TEMPO_FUGA) && stoCorreggendo)
    {
      ...
      {
        if (currentMillis - sensorePresoMillis >= 1000) lineeSetup();
      }
    }// while(currentMillis- correzioneMillis < TEMPO_FUGA && stoCorreggendo)
    lineeSetup();
  }// if(stoCorreggendo)
...
void calibraLinee()
{
  ...
  for (int lettura = 0; lettura < 36; lettura++)
  {
    soglia_Linee[lettura] = valMin[lettura] + 10;
...
bool lineaRilevata(byte nMux, int lettura)
{
  ...
  //Serial.print(nMux),Serial.print(" - "),Serial.println(lettura);
  
  if (analogRead(nPin) < soglia_Linee[lettura + (nMux * 12)])
  {
    //sensorePresoMillis = millis();
    return 1;
  }
  else return 0;
}
...
byte  bin[] = {B0000, B0001, B0010, B0011, B0100,B0101, B0110, B0111,B1000,B1001,B1010,B1011,B1100,B1101,B1110,B1111};//list of binary value
