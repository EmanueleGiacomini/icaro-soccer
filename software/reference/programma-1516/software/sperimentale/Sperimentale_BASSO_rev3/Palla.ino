...
void findPalla()
{
  Serial.println("--------find palla");
  contatoreMediaPalla++;
    //Serial.println(valPalla[0]);
  for (int x = 0; x <= 8; x++)
  {
    //Serial.print("Var n: ");
    //Serial.print(x);
...
byte posPalla() {  ////////////// trovo il valore minimo dell'array
  ...
  for (int i = 0; i  <= 19; i++) {
    if(i!=4&&i!=13&&i!=3)
    {
      ...
      }// DA TOGLIERE QUANDO SI RIPARA IL SENSORE 4 SU NEREA
  }
  if (minimo > 1000) palla = 99; /// non c'è la palla
  if(palla == 4) palla = 5; // In caso legge il sensore 4 
  if(palla == 13) palla = 12;
  if(palla == 3) palla = 2;


  if (minimo <= distanzaZero[palla]+5)     distanza = 0;
  else if (minimo <= distanzaUno[palla]+5) distanza = 1;
  else if (minimo <= distanzaDue[palla]+5) distanza = 2;
